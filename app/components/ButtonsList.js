  ...
  return (
    <div className={styles.wrapper}>
      <div className={styles.intro}>
        <p>
          Libraries (and library supporters) created and shared buttons with each other to promote
          summer reading in Michigan. This year's summer reading theme is <em>Unearth a Story</em>.
        </p>
      </div>
      ...
      {shuffleKey > 0 && (
        <div className={styles.buttons} key={shuffleKey}>
          {buttons.map((b, i) => (
            <Link href={`/${b.libraryId}?btn=${b.index}`} key={`${b.libraryId}_${b.index}`}>
              <Image
